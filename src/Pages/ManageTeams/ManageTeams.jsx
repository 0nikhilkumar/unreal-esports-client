...
function ManageTeams() {
  ...
  const filteredTeams = teamTiers.filter(
    (team) =>
      ...
      (selectedTier === "All" ||
        team.updateTeamTier[0]?.tier === selectedTier) // Check the tier inside updateTeamTier array
  );

  return (
  <div className="container w-full min-h-screen p-4 bg-gray-900 text-white">
    <h1 className="text-4xl font-bold my-14 mb-20 text-center">Team Management</h1>
    ...
    {/* Display message if no teams match the filter */}
    {filteredTeams.length === 0 && (
      <div className="bg-gray-800 p-6 rounded-lg shadow-lg text-center">
        <p className="text-xl text-gray-400">No teams found in this tier.</p>
      </div>
    )}
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      {filteredTeams.map((team) => {
        const currentTier =
          teamTiers.find((t) => t._id === team._id)?.tier || team.updateTeamTier[0].tier;
        return (
          <div key={team._id} className="bg-gray-800 rounded-lg overflow-hidden shadow-lg">
            <div className="p-6 cursor-pointer" onClick={() => toggleTeam(team._id)}>
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  >
                    <img
                      src={teamLogos[team._id] || team.logo || "/placeholder.svg"}
                  ...
                  <div>
                    ...
                    <div className="flex items-center mt-1">
                      ...
                      <span className={`font-semibold ${getTierColor(currentTier)}`}>
                        {currentTier}
                      </span>
                    </div>
                  </div>
            ...
            {expandedTeam === team._id && (
              <div className="bg-gray-700 p-6 rounded-b-lg">
                ...
                <div className="mt-4">
                  <label className="font-semibold text-white mr-4">Change Tier:</label>
                  <select
                    value={currentTier}
